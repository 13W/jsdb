...
function followPath(path, object, prevObj, prevKey, fullPath) {
    ...
    if (Array.isArray(object)) {
        var result = [],
            strPath = path.join('.');
        for (var i in object) {
            var tmpResult = followPath(strPath, object[i], object, i, followResult.path);
            if (tmpResult.value)
    ...
    var key = path.shift();
    return followPath(path, object[key], object, key, followResult.path);
}
...
function wrapMethod() {
    var method = [].shift.call(arguments),
        ...
        caller = [].pop.call(arguments);
    return function(data) {
        data = followPath(path, data);
        return method.call(context, data, query, queryObj, caller);
    }
}

function compileQuery(query, path, sequence, methods) {
    var self = this;
    methods = !methods && !Array.isArray(sequence);
    sequence = Array.isArray(sequence) || [];
    
    for (var key in query) {
        if (methods && key[0] === '$' && (key in methods)) {
            sequence.push(wrapMethod(methods[key], methods, query[key], query, path, self));
        } else 
        if ({}.toString.call(query[key]) == '[object Object]') {
            sequence.push(wrapMethod(function(data, query) {
                return query(data);
            }, null, query[key], compileQuery.call(self, query, key, methods), query, key, self))
        } else {
            sequence.push(wrapMethod(function(data, query) {
    ...
    return function(data) {
        for (var i in sequence) {
            if (!sequence[i](data)) return false;
        }
        return true;
    }
}
