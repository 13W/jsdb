...
function Cursor(collection, options) {
    var self = this;
...
Cursor.prototype = {
    projectionFuncs :   {
        $matchAll   :   function(data, value, property, object, self) {
            ...
            var result = [];
            for (var i in data) {
                var item = new Item(data[i], object);
    ...
    calculate       :   function() {
        ...
        for (var i in data) {
            ...
            var filterResult = data[i].check(query, this),
                resultData = this.options.items ? data[i] : data[i].get();
            
//            console.inspect(data[i], query, filterResult);
            if (!filterResult) continue;
            if (this.options.stream) {
                this.stream.emit('data', resultData);
                continue;
            }
            ...
            this.items.push(new Item(resultData, data[i]));
    ...
    forEach         :   function(callback) {
        if (!this.result) this.calculate();
        for(var i in this.result) this.options.items ? callback(this.items[i], i) : callback(extend({}, this.result[i]), i);
    },
    ...
    subCollection   :   function(path, options) {
        ...
        this.find(filter, column, options).forEach(function(e) {
            var items = e.getAtPath(path),
                subItems = Array.isArray(items) ? items.map(function(i) {return new Item(i, e)}) : new Item(items, e);
