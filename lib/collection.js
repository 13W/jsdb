...
var         QC = require('./queryCompiler').QueryCompiler,
...
function Collection(collectionName, options) {
    ...
    if (!Storage.exists(collectionName))
        Storage.createCollection(collectionName, options);

    Storage.associate(collectionName, this);
    self.data = Storage.get(collectionName);
...
Collection.prototype = {
    ...
    setDb           :   function(db) {
        this.db = db;
        this.indexes = this.db.getCollection('system.indexes');
    },
    new             :   function(collectionName, options) {
        var collection = new Collection(collectionName, options);
        collection.setDb(this.db);
        return collection;
    },
    ...
    count           :   function(query, callback) {
        ...
        if (!query) {
            if (callback) return callback(null, this.data.length);
        } else {
...
//        callback && console.log(callback.toString());
        if (!options.stream && typeof callback === 'function') return cursor.toArray(callback);
        return cursor;
    },
    ...
    insert          :   function(data, callback) {
        ...
        self.Storage.insert(this.collectionName, data, function(error, item) {
            ...
            self.jsItems.push(item.get());
        });
        if (typeof callback === 'function') callback(errors.length === 1? errors[0] : errors, Array.isArray(data) ? items : items[0]);
    },
    mapReduce       :   function(map, reduce, options, callback) {
        ...
        function mapFunction() {
            var normalizedMap = (map+"")
                    .replace(/\/\*(?!:\*\/)*\*\)/g, '')
                    .replace(/\/\/[^\n]*/g, '')
                    .replace(/\n|\r/g, '')
                    .replace(/[ \t]+/g, ' ');

            var func = (new Function('return new Function("'+Object.keys(scope).join('", "')+'", "return '+normalizedMap+'")'))();
            var args = [];
    ...
    _addToIndex     :   function(item) {
        ...
        function appendToIndex(index, indexKeys, item) {
            ...
            for (var i in indexKeys) {
                ...
                if (!~self._index[indexName][indexKeyName].indexOf(item)) {
                    if (index.unique && self._index[indexName][indexKeyName].length) {
                        return new Error('duplicate key error index: '+self.getFullName()+'.$'+indexName+'  dup key: '+indexKeyName);
                    }
        ...
        function createIndex(data) {
            for(var i in indexes) {
                ...
                var indexError = appendToIndex(indexes[i], self._createKeys(values), data);
                if (indexError instanceof Error) return indexError;
            }
            return null;
        }
        
        return createIndex(item) || null;
    },
    ...
    remove          :   function(query, options, callback) {
        var self = this;
        var Query = new QC(this, query, methods.$query);
        this.data.forEach(function(data) {
            if (Query.execute(data)===true) {
                self._delFromIndex(data);
    ...
    save            :   function(data, callback) {
        var self = this;
        self.Storage.insert(this.collectionName, data, function(error, item) {
            if (typeof callback === 'function' && error) return callback(error);
            if (error) return void 0;
            var indexError = self._addToIndex(item);
            if (indexError instanceof Error) {
                self.remove(item);
                if (typeof callback === 'function') callback(indexError);
                return void 0;
            }
    ...
    update          :   function(query, update, options, callback) {
        ...
        var Query = new QC(this, _query, methods.queries);
            this.data.forEach(function(data) {
                if (Query.execute(data)===true) {
                    self._updateIndex(data);
                }
            });
